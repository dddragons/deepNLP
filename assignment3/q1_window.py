...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        """
        for idx, item in enumerate(zip(sentence, labels)):
            ftrs = sentence[idx]
            for i in range(1, window_size + 1):
                if (idx - i) < 0:
                    ftrs = start + ftrs
                else:
                    ftrs = sentence[idx - i] + ftrs
                if (idx + i) > len(sentence) - 1:
                    ftrs.extend(end)
                else:
                    ftrs.extend(sentence[idx + i])
            windowed_data.append((ftrs, labels[idx]))
            #print windowed_data
        """
        ...
        for j, i in enumerate(range(window_size, N+window_size)):
            window_feats = sum(padded_sentence[i-window_size:i+window_size+1], [])
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", dtype=tf.float32, shape=[n_window_features * embedding_size, hidden_size],
        initializer = xavier_init)
        b1 = tf.get_variable("b1", dtype=tf.float32, shape=[hidden_size], initializer = xavier_init)
        U = tf.get_variable("U", dtype=tf.float32, shape=[hidden_size, n_classes],
        initializer = xavier_init)
        b2 = tf.get_variable("b2", dtype=tf.float32, shape=[n_classes], initializer = xavier_init)
