...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        cutLen = min(max_length, len(sentence))
        padLen = max(0, max_length - cutLen)

        paddedSentence = sentence[:cutLen] + [zero_vector] * padLen
        paddedLabels = labels[:cutLen] + [zero_label] * padLen
        mask = [True] * cutLen + [False] * padLen
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, max_length, n_features * embedding_size])
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable("U", shape=[Config.hidden_size, n_classes], initializer = xavier_init)
        b2 = tf.get_variable("b2", shape=[n_classes], initializer = xavier_init)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_dropout = tf.nn.dropout(o, dropout_rate)
                y = tf.add(tf.matmul(o_dropout, U), b2)
                preds.append(y)
        ...
        #preds = tf.reshape(preds, [-1, self.max_length, n_classes])
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = masked_labels,
        logits = masked_preds)
