...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable('W_x', shape = [input_size, state_size], initializer = xavier_init)
            b = tf.get_variable('b', shape=[state_size], initializer = xavier_init)
            W_h = tf.get_variable('W_h', shape=[state_size, state_size], initializer = xavier_init)

            z1 = tf.matmul(inputs, W_x)
            z2 = tf.matmul(state, W_h)
            z3 = tf.add(z1, z2)
            z4 = tf.add(z3, b)
            new_state = tf.nn.sigmoid(z4)
