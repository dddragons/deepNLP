...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_z = tf.get_variable('U_z', shape = [input_size, state_size], initializer = xavier_init)
            W_z = tf.get_variable('W_z', shape = [state_size, state_size], initializer = xavier_init)
            b_z = tf.get_variable('b_z', shape=[state_size], initializer = xavier_init)

            U_r = tf.get_variable('U_r', shape = [input_size, state_size], initializer = xavier_init)
            W_r = tf.get_variable('W_r', shape=[state_size, state_size], initializer = xavier_init)
            b_r = tf.get_variable('b_r', shape=[state_size], initializer = xavier_init)

            U_o = tf.get_variable('U_o', shape = [input_size, state_size], initializer = xavier_init)
            W_o = tf.get_variable('W_o', shape=[state_size, state_size], initializer = xavier_init)
            b_o = tf.get_variable('b_o', shape=[state_size], initializer = xavier_init)
            ...
            z3 = tf.matmul(inputs, U_o) + tf.multiply(r, tf.matmul(state, W_o)) + b_o
            o = tf.nn.tanh(z3)

            new_state = tf.multiply(z, state) + tf.multiply(tf.subtract(1.0, z), o)
